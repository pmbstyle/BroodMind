...
Octopal is a sophisticated AI agent platform built in Python. It is designed around a "Queen and Workers" architecture, where a central "Queen" orchestrates tasks and delegates them to specialized "Workers." The platform is highly modular, extensible, and designed for complex, multi-step tasks.

### Core Components

*   **Queen:** The central orchestrator of the Octopal platform. The Queen receives messages (from a Telegram bot), decides how to handle them, and dispatches tasks to Workers. It uses a large language model (LLM) to make decisions and communicate with the user.
...
*   **Telegram Bot:** The primary user interface for the Octopal platform. Users interact with the Queen through a Telegram bot.
