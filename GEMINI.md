...
This document provides a comprehensive overview of the BroodMind project, its architecture, and instructions for building, running, and developing the application.

## Project Overview

BroodMind is a sophisticated AI agent platform built in Python. It is designed around a "Queen and Workers" architecture, where a central "Queen" orchestrates tasks and delegates them to specialized "Workers." The platform is highly modular, extensible, and designed for complex, multi-step tasks.

### Core Components

*   **Queen:** The central orchestrator of the BroodMind platform. The Queen receives messages (from a Telegram bot), decides how to handle them, and dispatches tasks to Workers. It uses a large language model (LLM) to make decisions and communicate with the user.
*   **Workers:** Specialized agents that perform specific tasks. Workers are defined by a `WorkerSpec` and can be granted specific capabilities, such as network access or filesystem access. They can be run in the same environment as the Queen or as separate Docker containers.
...
*   **Memory:** A service that provides contextual memory to the Queen and Workers. This allows the agent to maintain a consistent state and remember previous interactions.
*   **Telegram Bot:** The primary user interface for the BroodMind platform. Users interact with the Queen through a Telegram bot.
*   **Gateway:** A web API (built with FastAPI) that provides an additional interface to the BroodMind platform.
...
3.  **Run the application:** The main entry point is the `broodmind start` command.
...
This will build the `broodmind` service and run the `broodmind start` command.
...
*   **Dependency Injection:** The `Queen` and `WorkerRuntime` classes are initialized with their dependencies, which makes the code easier to test and maintain.
