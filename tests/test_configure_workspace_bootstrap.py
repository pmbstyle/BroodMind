...
from broodmind.cli.configure import _ensure_workspace_markdown_files


def test_workspace_bootstrap_creates_required_markdown(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    result = _ensure_workspace_markdown_files(workspace)
    ...
    assert (workspace / "HEARTBEAT.md").read_text(encoding="utf-8") == ""
...
def test_workspace_bootstrap_is_non_destructive(tmp_path: Path) -> None:
    ...
    _ensure_workspace_markdown_files(workspace)
